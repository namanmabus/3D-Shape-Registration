...
% Reduce matrix.
A=A-min(A,[],2)*ones(1,size(A,1));
A=A-(min(A,[],1)'*ones(1,size(A,1)))';
...
for k=1:length(i)
    ...
    A(i(k),find(A(i(k),:)==-j(k)))=A(i(k),j(k));
    % Mark it as assigned.
    A(i(k),j(k))=0;
end
